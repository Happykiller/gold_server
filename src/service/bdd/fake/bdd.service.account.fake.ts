...
export class BddServiceAccountFake {
  ...
  getAccount(dto: GetAccountServiceDto): Promise<AccountServiceModel> {
    return Promise.resolve(
      this.collectionAccount.find((elt) => elt.id === dto.account_id),
    );
  }
  ...
  async updateAccount(
    ...
    const objIndex = this.collectionAccount.findIndex(
      (obj) => obj.id == dto.account_id,
    );
  ...
  async deleteAccount(dto: DeleteAccountServiceDto): Promise<boolean> {
    const elt = this.collectionAccount.find((obj) => obj.id == dto.account_id);
